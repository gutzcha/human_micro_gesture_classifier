...
def main(args):
    ...
    if args.save_path:
        save_folder_root = args.save_path
        save_folder = osp.join(save_folder_root, 'images')
    ...
    with torch.no_grad():
        ...
        for id, im in enumerate(imgs):
            im.save(f"{image_save_path}/ori_img{id}.jpg")
        
        image_array_list = [np.array(img) for img in imgs]
        output_path = f"{video_save_path}/ori_vid.mp4"
        ...
        for id, im in enumerate(imgs):
            im.save(f"{image_save_path}/rec_img{id}.jpg")
        
        image_array_list = [np.array(img) for img in imgs]
        output_path = f"{video_save_path}/rec_vid.mp4"
        # Write the images to an MP4 file
        writer = imageio.get_writer(output_path, fps=7)
        for image_array in image_array_list:
            writer.append_data(image_array)
        ...
        if torch.any(bool_masked_pos): # There is a mask
            ...
            for id, im in enumerate(imgs):
                im.save(f"{image_save_path}/mask_img{id}.jpg")
            
            image_array_list = [np.array(img) for img in imgs]
            output_path = f"{video_save_path}/mask_vid.mp4"
            # Write the images to an MP4 file
            writer = imageio.get_writer(output_path, fps=7)
            for image_array in image_array_list:
                writer.append_data(image_array)
