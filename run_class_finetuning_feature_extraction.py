...
#     parser.add_argument('--data_root', default='', type=str, help='path to training data to prepend to the file paths')
#
...
def main(args):
    # Define a forward hook to capture the output
    # Placeholder to store the output of the specific layer
    layer_outputs = []
    ...
    dataset_train, args.nb_classes = build_dataset(is_train=False, test_mode=False, args=args)
    ...
    with torch.no_grad():  # Disable gradient calculation for inference
        for batch in tqdm(data_loader_train, total=len(dataset_train)//total_batch_size):
            vid = batch[0]
            vid = vid.to(device)
            out = model(vid)
    ...
    # Convert the layer outputs to a NumPy array
    layer_outputs = torch.cat(layer_outputs, dim=0).cpu().numpy()

    # Create a DataFrame from the NumPy array
    df = pd.DataFrame(layer_outputs)
...
if __name__ == '__main__':
    ...
    args.finetune = (r'D:\Project-mpg microgesture\human_micro_gesture_classifier\experiments\mac_multi'
                     r'\split_loss_fine_coarse_70\checkpoint-best.pth')
